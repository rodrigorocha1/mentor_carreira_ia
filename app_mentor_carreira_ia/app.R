...
ui <- fluidPage(
    ...
    helpText("Sete os slides e clique em processar"),
    
    fluidRow(
        column(4, sliderInput("sexatas", "Gosto por Exatar", min = 5, max = 90, step = 10, value = 40)),
    ...
    fluidRow(
        column(4, sliderInput("slider", "Pefil de Liderança", min = 30, max = 95, step = 5, value = 50)),
    ...
    fluidRow(
        ...
        column(6, actionButton("Processar", "Processar"), helpText("A linha vermelha mostra a sua Aderência a profissão de ciência de dados"),
...
server <- function(input, output) {
    ...
    observeEvent(input$Processar,{
        inferencia = fuzzy_inference(sistema, list(Gosta_exatas = input$sexatas, Rel_interpess= input$sinter, Escre_codigo = input$scodigo, Perfil_lider = input$slider, Gosta_Estudar= input$sestudar,
                                                   Habilidade_comunicaco = input$scomunica))
